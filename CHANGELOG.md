...
- Toolchain current: TypeScript 7.0.2, `@types/node` 26, `actions/checkout@v7` and `actions/setup-node@v7`. TypeScript 7 no longer auto-includes `@types/node`, so `tsconfig.json` now names `types: ["node"]`; the emitted JavaScript and declaration files are byte-identical to what 5.9 produced. The type fixtures moved to their own project files, because 7 refuses command-line inputs while a `tsconfig.json` is in scope.
- A type fixture pins the portability promise from the consumer's side: a project with no Node types at all, importing only the main entry, has to typecheck.
- `verify`, `authorize` and `doctor` report a missing file as a missing file. A target that is plainly a path used to fall through to the domain branch, so a typo was answered with a complaint about hostnames.
